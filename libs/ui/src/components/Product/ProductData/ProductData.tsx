...
export const ProductData = ({ data }: Props) => {
  ...
  queryClient.invalidateQueries({ queryKey: [`products/${data.id}/bids`] })
  ...
  React.useEffect(() => {
    ...
    socket.on("Product-finished", (data) => {
      toast({
        title: "Aukcja",
    ...
    socket.on("Product-winner", (data) => console.log("WYGRAŁEŚ AUKCJE!", data))
    ...
    return () => {
      ...
      socket.off("bid-fail")
      socket.off("Product-finished")
      socket.off("Product-winner")
    }
  }, [socket, toast])
